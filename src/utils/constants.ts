...
export const postgresCA = process.env.POSTGRES_CA as string

export const redisConnectionString = process.env.REDIS_CONNECTION_STRING as string
export const redisCA = process.env.REDIS_CA as string
export const redisClientKey = process.env.REDIS_CLIENT_KEY as string
export const redisClientCert = process.env.REDIS_CLIENT_CERT as string
...
if (!postgresCA) throw new Error('`POSTGRES_CERTIFICATE_AUTHORITY` 환경 변수를 설정해주세요.')

if (!redisConnectionString) throw new Error('`REDIS_CONNECTION_STRING` 환경 변수를 설정해주세요.')
if (!redisCA) throw new Error('`REDIS_CA` 환경 변수를 설정해주세요.')
if (!redisClientKey) throw new Error('`REDIS_CLIENT_KEY` 환경 변수를 설정해주세요.')
if (!redisClientCert) throw new Error('`REDIS_CLIENT_CERT` 환경 변수를 설정해주세요.')
